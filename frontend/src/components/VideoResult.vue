<template>
  <div class="miku-card rounded-2xl border border-border shadow-lg overflow-hidden h-full">
    <!-- 视频信息头部 -->
    <div class="flex flex-col gap-5 p-5 sm:p-6">
      <div class="relative w-full aspect-video rounded-xl overflow-hidden bg-[#0a1f32]">
        <img
        ...
        <div v-if="video.duration_string" class="absolute bottom-2 right-2 px-2 py-0.5 bg-[#021420]/75 text-text-primary text-xs rounded-md border border-primary/20">
    ...
    <div class="border-t border-border-light px-5 sm:px-6 py-5">
        ...
        <button
          ...
          :class="[
            ...
            selectedFormat === fmt.format_id
              ? 'border-primary bg-primary/12 ring-1 ring-primary/20'
              : 'border-border-light hover:border-primary/40 hover:bg-primary/10'
          ]"
        >
          <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
            :class="selectedFormat === fmt.format_id ? 'bg-primary text-[#042a39]' : 'bg-[#112b42] text-text-muted'">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="mt-6 flex flex-col items-stretch gap-3">
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-10 rounded-full bg-primary hover:bg-primary-dark text-[#042a39] font-medium text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg cursor-pointer"
        >
