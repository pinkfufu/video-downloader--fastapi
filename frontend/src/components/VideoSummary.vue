<template>
  <div class="miku-card rounded-2xl border border-border shadow-lg overflow-hidden h-full flex flex-col">
        <!-- 标签页导航 -->
        ...
        <div class="p-5 sm:p-6 min-h-[400px] flex-1 overflow-y-auto">
          ...
          <div v-show="activeTab === 'summary'">
            ...
            <!-- 免费用户剩余次数提示 -->
            <div v-if="quotaInfo && quotaInfo.remaining >= 0 && !loading" class="mt-4 p-3 rounded-xl bg-primary/10 border border-primary/30 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="relative" ref="subtitleDropdownRef">
                    ...
                    <div
                      ...
                      class="absolute right-0 top-full mt-1 miku-card rounded-lg shadow-lg border border-border-light py-1 z-10 min-w-[120px]"
                    >
                      <button
                        v-for="fmt in subtitleFormats"
                        :key="fmt.key"
                        @click="downloadSubtitle(fmt.key)"
                        class="w-full text-left px-3 py-2 text-xs text-text-primary hover:bg-primary/10 transition-colors cursor-pointer flex items-center justify-between"
                      >
                        <span>{{ fmt.label }}</span>
          ...
          <div v-show="activeTab === 'mindmap'">
            <div v-if="mindmapMarkdown" class="relative">
              ...
              <div
                ...
                class="mindmap-wrapper w-full border border-border-light rounded-xl bg-[#0d2338] overflow-hidden"
              ...
              >
                ...
                <button
                  v-if="isFullscreen"
                  @click="toggleFullscreen"
                  class="fixed top-4 right-4 z-50 flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#0c2035]/95 backdrop-blur shadow-lg text-sm text-text-primary hover:bg-[#102844] transition-colors cursor-pointer border border-border-light"
                >
            ...
            <div class="space-y-4">
                  ...
                  <div
                    :class="[
                      ...
                      msg.role === 'user'
                        ...
                        : 'bg-[#122b42] text-text-primary rounded-bl-md border border-border-light'
                    ]"
                  >
              ...
              <div class="flex gap-2 pt-3 border-t border-border-light">
                <input
                  ...
                  type="text"
                  placeholder="输入你的问题..."
                  class="flex-1 h-11 px-4 rounded-xl border border-primary/30 bg-[#0f253a]/85 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                  :disabled="chatLoading"
                />
                <button
                  @click="sendQuestion"
                  :disabled="!chatInput.trim() || chatLoading"
                  class="h-11 px-5 rounded-xl bg-primary hover:bg-primary-dark text-[#042a39] text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center gap-1.5"
                >
