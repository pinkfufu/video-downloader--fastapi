...
def _map_subtitle_error(url: str, err: Exception) -> str:
    """
    将底层异常映射为前端可直接展示的友好提示。
    对 SSL 相关异常（尤其 Twitter）做统一文案，避免暴露冗长英文错误。
    """
    msg = str(err).lower()
    if "ssl" in msg or "unexpected_eof_while_reading" in msg or "ssLError".lower() in msg:
        if _is_twitter_url(url):
            return "当前平台字幕提取失败，暂不支持 AI 总结"
        return "当前平台字幕提取失败，请稍后重试"
    return str(err)
...
class SubtitleExtractor:
    ...
    def extract(self, url: str) -> dict:
        ...
        try:
            ...
            raise ValueError(_map_subtitle_error(url, e))
